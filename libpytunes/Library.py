...
class Library:
    def __init__(
        self, itunesxml, musicPathXML=None, musicPathSystem=None, filesOnly=False
    ):
    ...
    def getSongs(self):
        ...
        for trackid, attributes in self.il["Tracks"].items():
            ...
            if attributes.get("Location"):
                ...
                location = urlparse.unquote(
                    urlparse.urlparse(attributes.get("Location")).path
                )
                location = location
            ...
            s = Song(
                ...
                movement_number=(
                    int(attributes.get("Movement Number"))
                    if attributes.get("Movement Number")
                    else None
                ),
                movement_count=(
                    int(attributes.get("Movement Count"))
                    if attributes.get("Movement Count")
                    else None
                ),
                movement_name=attributes.get("Movement Name"),
                track_id=(
                    int(attributes.get("Track ID"))
                    if attributes.get("Track ID")
                    else None
                ),
                ...
                total_time=(
                    self._ms_to_time_str(attributes.get("Total Time"))
                    if attributes.get("Total Time")
                    else None
                ),
                start_time=(
                    self._ms_to_time_str(attributes.get("Start Time"))
                    if attributes.get("Start Time")
                    else None
                ),
                stop_time=(
                    self._ms_to_time_str(attributes.get("Stop Time"))
                    if attributes.get("Stop Time")
                    else None
                ),
                track_number=(
                    int(attributes.get("Track Number"))
                    if attributes.get("Track Number")
                    else None
                ),
                track_count=(
                    int(attributes.get("Track Count"))
                    if attributes.get("Track Count")
                    else None
                ),
                disc_number=(
                    int(attributes.get("Disc Number"))
                    if attributes.get("Disc Number")
                    else None
                ),
                disc_count=(
                    int(attributes.get("Disc Count"))
                    if attributes.get("Disc Count")
                    else None
                ),
                year=int(attributes.get("Year")) if attributes.get("Year") else None,
                date_modified=(
                    time.strptime(str(attributes.get("Date Modified")), dt_format)
                    if attributes.get("Date Modified")
                    else None
                ),
                date_added=(
                    time.strptime(str(attributes.get("Date Added")), dt_format)
                    if attributes.get("Date Added")
                    else None
                ),
                bit_rate=(
                    int(attributes.get("Bit Rate"))
                    if attributes.get("Bit Rate")
                    else None
                ),
                sample_rate=(
                    int(attributes.get("Sample Rate"))
                    if attributes.get("Sample Rate")
                    else None
                ),
                comments=attributes.get("Comments"),
                rating=(
                    int(attributes.get("Rating")) if attributes.get("Rating") else None
                ),
                rating_computed="Rating Computed" in attributes,
                play_count=(
                    int(attributes.get("Play Count"))
                ...
                lastplayed=(
                    time.strptime(str(attributes.get("Play Date UTC")), dt_format)
                    if attributes.get("Play Date UTC")
                    else None
                ),
                skip_count=(
                    int(attributes.get("Skip Count"))
                    if attributes.get("Skip Count")
                    else None
                ),
                skip_date=(
                    time.strptime(str(attributes.get("Skip Date")), dt_format)
                    if attributes.get("Skip Date")
                    else None
                ),
                length=(
                    int(attributes.get("Total Time"))
                    if attributes.get("Total Time")
                    else None
                ),
                ...
                release_date=(
                    time.strptime(str(attributes.get("Release Date")), dt_format)
                    if attributes.get("Release Date")
                    else None
                ),
    ...
    def albums(self):
        return self.return_grouped(lambda x: x.album)

    def artists(self):
        return self.return_grouped(lambda x: x.artist)

    def return_grouped(self, key: lambda x: x):
        grouped = {}
        for i, s in self.songs.items():
            if key(s) not in grouped:
                grouped[key(s)] = [s]
            else:
                grouped[key(s)].append(s)

        return grouped
        # for some reason groupby wasn't working
        ...
        for playlist in self.il["Playlists"]:
            if playlist["Name"] == playlistName:
                ...
                p = Playlist(
                    ...
                    is_genius_playlist=(
                        True if playlist.get("Genius Track ID") else False
                    ),
                    is_smart_playlist=(
                        True
                        if playlist.get("Smart Info")
                        and not playlist.get("Folder", False)
                        else False
                    ),
                )
                tracknum = 1
            ...
            for playlist in self.il["Playlists"]:
                if playlist["Name"] == playlistName:
                    ...
                    for track in playlist["Playlist Items"]:
                        id = int(track["Track ID"])
                        x.add_track(
                            title=self.songs[id].name,
                            creator="",
                            location=self.songs[id].location,
                        )
                    return x.toXml()
        else:
            logger.warning(
                "xspf library missing, go to https://github.com/alastair/xspf to install."
            )
            return None
